...
async function fetchVerifiedCentersFromDb() {
  await connectDB();

  const docs = await Centerlist.find({ isVerified: true })
    .select(
...
export const getVerifiedCenters = unstable_cache(
  async () => {
    try {
      return await fetchVerifiedCentersFromDb();
    } catch (error) {
      console.error("getVerifiedCenters failed:", error);
      return [];
    }
  },
  ["getVerifiedCenters-v2"],
  { revalidate: 60 }
);
